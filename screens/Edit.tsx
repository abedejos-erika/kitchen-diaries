...
import { db, storage, ref, collection, addDoc, getDownloadURL, uploadBytesResumable, doc, updateDoc } from "../firebase";

export default function Edit({ navigation, route }: any) {
  ...
  const submit = async (values: any, actions: any) => {
    ...
    if (image) {
      setLoad(true);
      await updateDoc(doc(db, "recipes", recipe.id), { title, image, desc, link })
        .then(() => {
          setImage("");
          setLoad(false);
          actions.resetForm();
      ...
      <View style={styles.container}>
        <View style={{ width: "100%", height: "100%" }}>
          <View style={styles.titleContainer}>
            ...
            <Text style={styles.titletextContainer}>Edit Recipe</Text>
          </View>

          <Formik
            onSubmit={submit}
            validationSchema={recipeFormSchema}
            initialValues={{ desc: recipe.desc || "", link: recipe.link || "", title: recipe.title || "" }}
          >
